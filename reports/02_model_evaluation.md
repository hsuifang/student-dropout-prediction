...
- **Total Loss** = Classification Loss (Focal Loss) + $\lambda$ × Fairness Loss (MMD Loss)；$\lambda$ (即代碼中的 $\beta$) = `0.4821` *(此為 Optuna 在 5-Fold 交叉驗證下，歷經 15 次貝氏嘗試所尋獲之兼顧準確度與公平性的黃金客觀超參數)*。
